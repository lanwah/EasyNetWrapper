...
namespace EasyNet.Core.Security.CRC
{
    ...
    public static partial class Crc
    {
        ...
        /// <returns>CRC32校验码</returns>
        public static UInt32 ComputeCrc32(this byte[] buffer)
        {
            return (new Crc32Provider()).Compute(buffer);
        }
        /// <summary>
        /// 从字节数组中生成32位CRC校验码
        /// </summary>
        /// <param name="buffer">要计算CRC32的输入</param>
        /// <param name="offset">字节数组中的偏移量，从该位置开始使用数据</param>
        /// <param name="count">数据中用作数据的字节数</param>
        /// <returns>CRC32校验码</returns>
        public static UInt32 ComputeCrc32(this byte[] buffer, int offset, int count)
        {
            return (new Crc32Provider()).Compute(buffer, offset, count);
        }
        /// <summary>
        /// 从System.IO.Stream中生成32位CRC校验码
        /// </summary>
        /// <param name="inputStream">System.IO.Stream</param>
        /// <returns>CRC32校验码</returns>
        public static UInt32 ComputeCrc32(this Stream inputStream)
        {
            return (new Crc32Provider()).Compute(inputStream);
        }
    }
}
